...
const DB = (async () => {
  try { return (await import('bun:sqlite')).Database; }
  catch { return (await import('node:sqlite')).DatabaseSync; }
})();
...
async function queryUsage(dbPath?: string): Promise<UsageStats | string> {
  const SQLite = await DB;
  const path = dbPath ?? join(homedir(), '.local', 'share', 'opencode', 'opencode.db');
  let db: InstanceType<typeof SQLite>;
  try {
    db = new SQLite(path, { readonly: true } as Record<string, unknown>);
  } catch {
...
export function renderProgressBar(
  current: number, limit: number, label: string, termWidth = 80
): string {
  ...
  return `${color}${label}: ${BOLD}${FULL.repeat(filled)}${DIM}${EMPTY.repeat(empty)}${RESET}${color} ${curStr} / ${limStr} (${pctStr})${RESET}`;
}
...
export function renderUsage(stats: UsageStats, termWidth = 80): string {
  ...
  const diff = stats.reset_time.getTime() - Date.now();
...
export async function runUsage(dbPath?: string): Promise<string> {
  const stats = await queryUsage(dbPath);
